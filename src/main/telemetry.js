...
module.exports = function registerTelemetry({ si, formatBytes, safeSend, powerMonitor, getConfig }) {
  ...
  function startPolling(mainWindow) {
    ...
    const poll = async () => {
      ...
      _pollCount++;
      try {
        // Core stats every tick (15s) — lightweight
        const tasks = [si.currentLoad(), si.mem()];

        // Disk + network only every 3rd tick (~45s) — these change slowly
        const doDiskNet = (_pollCount % 3 === 0);
        if (doDiskNet) {
          tasks.push(si.fsSize(), si.networkStats());
        }

        // Temp every 6th tick (~90s) — WMI is very expensive on Windows
        ...
        let rIdx = 2;
        if (doDiskNet) {
          _lastDisk = results[rIdx].status === 'fulfilled' ? results[rIdx].value : [];
          rIdx++;
          _lastNet = results[rIdx].status === 'fulfilled' ? results[rIdx].value : [];
          rIdx++;
        }
        if (doTemp) {
          const t = results[rIdx].status === 'fulfilled' ? results[rIdx].value : null;
